...
const SUMMARIES: &[(&str, &str)] = &[
    ...
    (
        "cwai",
        "AND condition codes, stack the machine state, wait for interrupt",
    ),
...
pub static SET: &[Insn] = &[
    ...
    Insn::mem("adca", op![0x89], op![0x99], op![0xA9], op![0xB9], 1),
    Insn::mem("adcb", op![0xC9], op![0xD9], op![0xE9], op![0xF9], 1),
    ...
    Insn::mem("sbca", op![0x82], op![0x92], op![0xA2], op![0xB2], 1),
    Insn::mem("sbcb", op![0xC2], op![0xD2], op![0xE2], op![0xF2], 1),
    ...
    Insn::mem("bita", op![0x85], op![0x95], op![0xA5], op![0xB5], 1),
    Insn::mem("bitb", op![0xC5], op![0xD5], op![0xE5], op![0xF5], 1),
    ...
    // --- condition-code register (immediate only) ----------------------------
    Insn::mem("andcc", op![0x1C], &[], &[], &[], 1),
    Insn::mem("orcc", op![0x1A], &[], &[], &[], 1),
    // `cwai` ANDs the condition codes, stacks the full machine state and waits
    // for an interrupt. Immediate only, like the two above it.
    Insn::mem("cwai", op![0x3C], &[], &[], &[], 1),
    ...
    Insn::inh("swi2", op![0x10, 0x3F]),
    Insn::inh("swi3", op![0x11, 0x3F]),
